...
try {
    ...
    $sql = "SELECT 
            ...
            JOIN tlicense tl ON tl.tlicensecode = li.codelicense
            WHERE w.workercode = :workercode 
            AND li.codelicense = :codelicense 
            AND tl.approval_status = '1'";

    $stmt = $conn->prepare($sql);
    $stmt->bindParam(':workercode', $workercode, PDO::PARAM_STR);
    $stmt->bindParam(':codelicense', $codelicense, PDO::PARAM_INT);
    $stmt->execute();

    $license = $stmt->fetch(PDO::FETCH_ASSOC);

    if(!$license) {
        throw new Exception("No se encontró el permiso solicitado");
    }
